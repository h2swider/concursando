...
class UserModel {
    ...
    public function guardarUsuario($data) {
        try {
            ...
            $fecha_formateada = $fecha_nacimiento->format('Y-m-d H:i:s');
            ...
            $query->bindParam(":fn", $fecha_formateada, PDO::PARAM_STR);
            $query->execute();
    ...
    public function savePass($id, $pass) {
        ...
        try {
            $pass = md5($pass . SALT);
            $sql = "update usuario set pass = :p where id_usuario = :i";
            $query = $this->conexion->prepare($sql);
